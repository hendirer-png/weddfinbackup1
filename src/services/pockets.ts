...
export async function closePocketBudget(id: string): Promise<{ transaction: any }> {
  // Transfer all funds back to source card and delete pocket
  const { data: pocket, error: fetchErr } = await supabase
    .from(TABLE)
    .select('*')
    .eq('id', id)
    .single();
  ...
  // This should ideally involve creating an income transaction for the card
  // and then deleting the pocket. For simplicity:
  await deletePocket(id);

  return { transaction: { id: 'temp', amount: pocket.amount, description: `Penutupan ${pocket.name}` } };
}
