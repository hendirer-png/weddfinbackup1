...
import { validateCardBalance } from '@/services/balanceValidator';
...
export async function updatePocketBalance(pocketId: string, delta: number): Promise<void> {
  const POCKETS = 'pockets';
  // increment balance = balance + delta
  const { error } = await supabase.rpc('increment_pocket_balance', { p_pocket_id: pocketId, p_delta: delta });
  if (error) {
    // fallback: do update with select-then-update
    const { data: pocket, error: selErr } = await supabase.from(POCKETS).select('amount').eq('id', pocketId).maybeSingle();
    if (selErr) throw selErr;
    const current = Number(pocket?.amount || 0);
    const { error: updErr } = await supabase.from(POCKETS).update({ amount: current + delta }).eq('id', pocketId);
    if (updErr) throw updErr;
    return;
  }
}
...
export async function transferFunds(params: {
  ...
  const { fromCardId, fromPocketId, toCardId, toPocketId, amount, isCashTopup } = params;
